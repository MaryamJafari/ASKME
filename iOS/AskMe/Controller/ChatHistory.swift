...
class ChatHistory: UIViewController,  UITableViewDelegate, UITableViewDataSource, ChatHistoryDelegate{
                                    ...
                                    if let senderImageUrl = data[Constant.SENDERPHOTOURL] as? String{
                                        if let receiverImageUrl = data[Constant.RECIVERPHOTOURL] as? String{
                                            if let text = data[Constant.TEXT] as? String{
                                                if let notification = data[Constant.NOTIFICATION_TOKEN] as? String{
                                                    if let reciverId = data[Constant.RECIVER_ID] as? String{
                                                        let message = Message(senderName: senderName, receiverName: recieverName, recieverid: reciverId, imageURL: senderImageUrl, senderId: senderID, notification: notification, message: text, date: date, reciverImageURL: receiverImageUrl)
                                ...
                                if let  date = data[Constant.DATE] as? NSNumber{
                                    if let senderImageUrl = data[Constant.SENDERPHOTOURL] as? String{
                                        if let receiverImageUrl = data[Constant.RECIVERPHOTOURL] as? String{
                                            if let notification = data[Constant.NOTIFICATION_TOKEN] as? String{
                                                if let reciverId = data[Constant.RECIVER_ID] as? String{
                                                    let message = Message(senderName: senderName, receiverName: recieverName, recieverid: reciverId, imageURL: senderImageUrl, senderId: senderID, notification: notification, message: "Image or Video", date: date, reciverImageURL: receiverImageUrl)
    ...
    @IBAction func logOut(_ sender: Any) {
        if AuthProvider.Instance.logOut(){
            DBProvider.Instance.updateNotificationToken(userID: ID, token: "")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (newMessage[indexPath.row].senderId == Auth.auth().currentUser?.uid){
            DBProvider.Instance.getNotificationToken(userId : newMessage[indexPath.row].recieverId) { (notificationToken) in
            ...
            DBProvider.Instance.getNotificationToken(userId : newMessage[indexPath.row].senderId) { (notificationToken) in
                self.notification = notificationToken
