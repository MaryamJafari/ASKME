...
class RTCVideoChatViewController: UIViewController,RTCEAGLVideoViewDelegate,ARDAppClientDelegate, PNObjectEventListener, HangupFromDelegate {
    ...
    var   isAudioOnly:Bool = false; // Stephen @04/04/2018
    var   senderName : String = "defaultSenderName"; // Stephen @04/05/2018
    var   reciverName : String = "defaultReciverName"; // Stephen @04/05/2018
    ...
    override func viewDidLoad() {
        ...
        appDelegate.client.subscribeToChannels(["my_channel"], withPresence: true) // Subscribe to demo channel with
        appDelegate.startPageAddr?.RTCdelegate = self
        appDelegate.startPageAddr?.isOnCall = true
        appDelegate.startPageAddr?.roomName = self.roomName! as String
        print("RTCVideo my email is \(String(describing: appDelegate.startPageAddr?.myEmail))")
        // Stephen <=========
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        client!.connectToRoom(withId: self.roomName! as String!, settings: settingsModel, isLoopback: false, isAudioOnly: isAudioOnly, shouldMakeAecDump: false, shouldUseLevelControl: false)
    ...
    func disconnect(){
        if let _ = self.client{
            ...
            // Stephen ========>
            let signal = CallSignal(timestamp:Date(), sender:senderName, receiver:reciverName, action:"disconnect", response:"disconnect", roomName:self.roomName as! String);
            ...
            appDelegate.startPageAddr?.isOnCall = false;
            // <======== Stephen
        }
    }
